...
def download_pdf(request):
    ...
    title = Paragraph("LettuceCare Disease Prediction Report", styles['Title'])
    elements.append(title)
    elements.append(Spacer(1, 20))  

    table_data = [['Type', 'Probability (%)', 'Recommendation', 'Timestamp']]  # Header row
    ...
    if history_records.exists():
        for entry in history_records:
            table_data.append([
                ...
                f"{entry.probability:.2f}%",  # Convert to percentage
        ...
        table.setStyle(TableStyle([
            ...
            ('ROWBACKGROUNDS', (0, 1), (-1, -1), [colors.whitesmoke, colors.lightgrey]),  # Alternating rows
